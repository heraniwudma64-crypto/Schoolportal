...
import { Search, RefreshCw, FileCheck, CheckCircle, XCircle, Eye } from 'lucide-react';
...
const AdminReportCards = () => {
  ...
  // Mock data for report card submissions
  const mockSubmissions = [
    { id: 1, section: 'Grade 10A', teacher: 'Alemu', submittedDate: 'Aug 27, 2026', status: 'Pending Review', studentsCount: 25 },
    { id: 2, section: 'Grade 10B', teacher: 'Kebede', submittedDate: 'Aug 27, 2026', status: 'Submitted', studentsCount: 27 },
    { id: 3, section: 'Grade 10C', teacher: 'Abebe', submittedDate: 'Aug 26, 2026', status: 'Approved', studentsCount: 24 },
    { id: 4, section: 'Grade 9A', teacher: 'Chala', submittedDate: null, status: 'Draft', studentsCount: 30 },
  ];
  ...
  const handleReviewClick = (section: any) => {
    if (!termId) {
      alert("Please select a Term first to review report cards.");
      return;
    }
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500">Review and approve term report cards submitted by Home Room Teachers.</p>
        </div>
              ...
              {filteredSubmissions.length > 0 ? (
                filteredSubmissions.map((sub) => (
                  <tr key={sub.id} className="hover:bg-gray-50/50 transition-colors group">
                    ...
                    <td className="px-6 py-4 text-sm font-medium text-gray-500">{sub.studentsCount}</td>
                    <td className="px-6 py-4">
                      <span className={cn("px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest flex items-center gap-1 w-max",
                        ...
                        (sub.status === 'Pending Review' || sub.status === 'Submitted') ? "bg-amber-100 text-amber-700" :
                    ...
                    <td className="px-6 py-4 text-right">
                      <button 
                        ...
                        disabled={!termId}
                        className="px-4 py-2 bg-blue-50 text-blue-700 hover:bg-blue-100 rounded-lg text-xs font-bold uppercase tracking-widest transition-colors inline-flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                      >
          ...
          <div className="bg-white rounded-[2rem] w-full max-w-4xl max-h-[90vh] shadow-2xl flex flex-col overflow-hidden">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50">
              <div>
                <h3 className="text-xl font-black text-gray-900">
                  Review Report Cards: {selectedSection.section}
                </h3>
                <p className="text-sm text-gray-500 mt-1">
                  Submitted by {selectedSection.teacher} • Term: {terms.find(t => t.id === termId)?.name || 'N/A'} • {selectedSection.studentsCount} Students
                </p>
              </div>
              <span className={cn("px-4 py-2 rounded-full text-xs font-black uppercase tracking-widest",
                 ...
                 (selectedSection.status === 'Pending Review' || selectedSection.status === 'Submitted') ? "bg-amber-100 text-amber-700" :
            ...
            <div className="p-6 overflow-y-auto flex-1">
              <div className="border border-gray-200 rounded-2xl p-8 text-center text-gray-500">
                ...
                <p className="font-bold text-gray-900">Report Cards Preview</p>
                <p className="text-sm mt-2">This is a frontend placeholder for reviewing generated report cards.</p>
                <p className="text-xs text-gray-400 mt-1">The detailed report cards for the {selectedSection.studentsCount} students will be displayed here once connected to the backend.</p>
              </div>
            </div>
            ...
            <div className="p-6 border-t border-gray-100 flex justify-between items-center bg-gray-50">
              ...
              {(selectedSection.status === 'Pending Review' || selectedSection.status === 'Submitted') && (
